...
fn generated_split_vardct_exposes_global_cursor_when_available() {
    ...
    assert_eq!(
        spatial_with_dc_grid
            ...
            .collect::<Vec<_>>(),
        vec![
            (49152, 9746086918519815509),
            (49152, 17440078846890755999),
            (49152, 1396780167515462080),
        ]
    );
    assert_eq!(
    ...
    assert_eq!(
        spatial_grid
            ...
            .collect::<Vec<_>>(),
        vec![
            (39513, 4239850168503413225),
            (49054, 9059697401613785067),
            (49152, 7466818102430492487),
        ]
    );
    assert_eq!(
    ...
    assert_eq!(
        xyb_summary,
        vec![
            (61440, 4776126936367490909),
            (61440, 16966360282516586538),
            (61440, 1742708281763828599),
        ]
    );
    assert_eq!(
        xyb_anchors,
        [
            3099662244, 1003511080, 1002469964, 969605886, 1053810584, 1053949846
        ]
    );
    ...
    assert_eq!(
        dc_xyb_summary,
        vec![
            (61410, 3168490428156493259),
            (61440, 3935367174704808794),
            (61440, 16800706604491707441),
        ]
    );
    assert_eq!(
        dc_xyb_anchors,
        [
            3099662244, 1005673870, 1004092058, 972119438, 1053756378, 1053904672
        ]
    );
    ...
    assert_eq!(
        rgb_summary,
        vec![
            (61440, 10627684673353247952),
            (61440, 5089794051051329863),
            (61440, 1065709725921335673),
        ]
    );
    assert_eq!(
        rgb_anchors,
        [
            967892560, 974057666, 971940720, 1043924355, 1043220975, 1043814002
        ]
    );
    ...
    assert_eq!(
        ...
        vec![
            (
                ...
                1044791867,
                1131648160,
                17039629977098847119,
                vec![3099662244, 3146932199, 969605886],
            ),
            (
                61440,
                0,
                ...
                1190538327,
                9560992943873872785,
                vec![1003511080, 1045312429, 1053810584],
            ),
            (
                61440,
                0,
                ...
                1190616908,
                3869265576747241629,
                vec![1002469964, 1040753511, 1053949846],
            ),
        ]
    );
    assert_eq!(
        ...
        vec![
            (
                ...
                1133696090,
                1216755966,
                16679180194556774205,
                vec![967892560, 1008569276, 1043924355],
            ),
            (
                61440,
                1708,
                ...
                1134099216,
                1208404130,
                10865324924528921953,
                vec![974057666, 1030726375, 1043220975],
            ),
            (
                61440,
                244,
                ...
                1212391878,
                6692378734389440760,
                vec![971940720, 1010156324, 1043814002],
            ),
        ]
    );
    ...
    assert_eq!(metrics.sum_abs_error, 7696330);
    assert_eq!(metrics.checksum, 17131955763813530032);
    assert_eq!(metrics.anchors, vec![1, 2, 1, 255, 255, 255, 118, 115, 117]);
    assert_eq!(
    ...
    assert_eq!(
        ...
        vec![
            (
                ...
                255,
                7696330,
                17131955763813530032,
                vec![1, 2, 1, 255, 255, 255, 118, 115, 117],
                vec![0, 1, 1, 125, 128, 124, 253, 255, 255],
            ),
            (
            ...
            (
                ...
                255,
                13505687,
                13700408499968969178,
                vec![6, 6, 0, 255, 255, 0, 135, 132, 0],
                vec![0, 1, 1, 125, 128, 124, 253, 255, 255],
            ),
        ]
    );
    ...
    assert_eq!(dc_metrics.sum_abs_error, 7771864);
    assert_eq!(dc_metrics.checksum, 10439517315924058198);
    assert_eq!(
        dc_metrics.anchors,
        vec![1, 2, 2, 255, 255, 255, 118, 114, 117]
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        (
            181631,
            10439517315924058198,
            vec![1, 2, 2, 24, 68, 27, 118, 114, 117],
        )
    );
            ...
            (
                ...
                vec![
                    (
                        ...
                        960567205,
                        767,
                        13293848732014236376,
                        vec![3108050853, 3155812055, 3148609807],
                    ),
                    (
                        1,
                        985733029,
                        768,
                        8195250752669790513,
                        vec![1014062478, 1053286778, 1061823681],
                    ),
                    (
                        2,
                        994121637,
                        768,
                        8696829327620298818,
                        vec![1012480665, 1048837930, 1061082206],
                    ),
                ],
            ),
            (
                ...
                vec![
                    (
                        ...
                        960567205,
                        192,
                        16595434841318851770,
                        vec![1018496555, 1007735228, 960567205],
                    ),
                    (
                        1,
                        985733029,
                        192,
                        4753613839941866025,
                        vec![1054028252, 1057807360, 1062528082],
                    ),
                    (
                        2,
                        994121637,
                        192,
                        17845862838213260255,
                        vec![1056796424, 1058870140, 1062577514],
                    ),
                ],
            ),
        ]
...
#[test]
fn generated_flat_vardct_prefix_ac_decodes_when_available() {
    let Some(cjxl) = reference_cjxl() else {
        eprintln!("skipping generated flat VarDCT prefix AC fixture; reference cjxl is not built");
        return;
    };

    let input = unique_temp_path("jxl-rs-vardct-prefix-source", "ppm");
    let encoded = unique_temp_path("jxl-rs-vardct-prefix", "jxl");
    write_flat_vardct_source_ppm(&input);
    ...
    assert!(
        plan.frame.is_combined,
        "flat VarDCT fixture unexpectedly used split sections"
    );

    let ac_global = plan.ac_global_metadata.as_ref().unwrap();
    assert_eq!(ac_global.passes.len(), 1);
    assert_eq!(ac_global.passes[0].use_prefix_code, Some(true));
    assert_eq!(ac_global.parse_error, None);
    assert_eq!(plan.ac_group_metadata.len(), 1);

    let group = &plan.ac_group_metadata[0];
    assert_eq!(group.entropy_uses_prefix_code, Some(true));
    assert_eq!(group.cursor.ans_state_start_bits, None);
    assert_eq!(group.cursor.ans_state_end_bits, None);
    assert_eq!(group.cursor.coefficient_stream_start_bits, Some(463));
    assert_eq!(group.cursor.modular_ac_start_bits, Some(463));
    assert_eq!(group.parse_error, None);

    let probe = group.coefficient_probe.as_ref().unwrap();
    assert_eq!(probe.nzeros, 0);
    assert_eq!(probe.coefficient_events.len(), 0);
    assert_eq!(probe.coefficient_event_checksum, 14695981039346656037);
    assert_eq!(
        group
            .coefficient_summary
            .as_ref()
    ...
    assert!(group.spatial_with_dc_grid.is_some());

    let image = assemble_vardct_srgb8_image(plan).unwrap().unwrap();
    assert_eq!(image.width, 64);
    assert_eq!(image.height, 64);
    assert_eq!(
        srgb8_image_summary(&image),
        (
            12288,
            8125193502599957688,
            vec![17, 17, 17, 17, 17, 17, 17, 17, 17]
        )
    );
}
...
fn generated_vardct_xyb_inverse_variants_compare_against_oracle_when_available() {
    ...
    assert!(sweep.iter().all(|(_, variants)| {
        variants
            ...
            == Some(VarDctXybInverseVariant::BMinusBias)
    }));
}
...
fn generated_progressive_ac_vardct_exposes_pass_payloads_when_available() {
    ...
    assert_eq!(
        plan.ac_group_metadata
            ...
            .collect::<Vec<_>>(),
        vec![
            (0, 0, 11952, Some(32), Some(11951), None),
            (0, 1, 2912, Some(32), Some(2909), None),
            (1, 0, 8384, Some(32), Some(8377), None),
            (1, 1, 1888, Some(32), Some(1888), None),
            (2, 0, 17968, Some(32), Some(17964), None),
            (2, 1, 4000, Some(32), Some(3996), None),
        ]
    );
...
fn generated_vardct_nondefault_qf_context_map_decodes_coefficients_when_available() {
    ...
    assert_eq!(metrics.max_abs_error, 242);
    assert_eq!(metrics.sum_abs_error, 175_552_247);
    assert_eq!(metrics.checksum, 9247824429328187165);
    assert_eq!(metrics.anchors, vec![21, 132, 66, 51]);
...
fn generated_vardct_intensity_target_scales_opsin_plan_when_available() {
    ...
    let metrics = srgb8_oracle_metrics(&srgb8_image, &reference, &anchor_indices);
    assert_eq!(metrics.max_abs_error, 255);
    assert_eq!(metrics.sum_abs_error, 8129624);
    assert_eq!(metrics.checksum, 115631842491514046);
    assert_eq!(metrics.anchors, [1, 1, 1, 255, 255, 255, 114, 110, 113]);
    assert_eq!(
        metrics.reference_anchors,
        [0, 0, 0, 127, 126, 124, 253, 255, 255]
    );
}
...
fn generated_vardct_no_gaborish_skips_filter_when_available() {
    ...
    assert_eq!(
        xyb_summary,
        vec![
            (61440, 15461035939259390132),
            (61440, 17209967685753724715),
            (61440, 15886226285870294397),
        ]
    );
    assert_eq!(
        xyb_anchors,
        [
            3099662244, 1002122379, 1000868846, 968955812, 1053694834, 1053788049
        ]
    );
    assert_eq!(
        rgb_summary,
        vec![
            (61440, 10569589599704128632),
            (61440, 10639762418642730909),
            (61440, 14145313576586400500),
        ]
    );
    assert_eq!(
        rgb_anchors,
        [
            966226910, 973196379, 969684104, 1043690144, 1043029324, 1043429424
        ]
    );
    assert_eq!(metrics.max_abs_error, 255);
    assert_eq!(metrics.sum_abs_error, 8521559);
    assert_eq!(metrics.checksum, 812234670456358170);
    assert_eq!(metrics.anchors, vec![1, 2, 1, 59, 56, 58, 117, 114, 116]);
    assert_eq!(
        metrics.reference_anchors,
...
fn generated_vardct_epf_three_runs_all_filter_passes_when_available() {
    ...
    assert_eq!(
        xyb_summary,
        vec![
            (61440, 3129841762515575087),
            (61440, 5222151597635897858),
            (61440, 1410855049308821424),
        ]
    );
    assert_eq!(
        xyb_anchors,
        [
            3099662245, 1003732565, 1002636078, 970197614, 1053795635, 1053937387
        ]
    );
    assert_eq!(
        rgb_summary,
        vec![
            (61440, 74230751137887495),
            (61440, 58431518462262867),
            (61440, 4498591432367451326),
        ]
    );
    assert_eq!(
        rgb_anchors,
        [
            968178088, 974204886, 972084096, 1043917384, 1043183860, 1043787466
        ]
    );
    assert_eq!(metrics.max_abs_error, 255);
    assert_eq!(metrics.sum_abs_error, 7681385);
    assert_eq!(metrics.checksum, 16316801854211866216);
    assert_eq!(metrics.anchors, vec![1, 2, 2, 255, 255, 255, 118, 114, 117]);
    assert_eq!(
        metrics.reference_anchors,
...
fn generated_vardct_epf_disabled_keeps_gaborish_when_available() {
    ...
    let input = unique_temp_path("jxl-rs-vardct-no-epf-source", "ppm");
    let encoded = unique_temp_path("jxl-rs-vardct-no-epf", "jxl");
    let reference_output = unique_temp_path("jxl-rs-vardct-no-epf-reference", "ppm");
    write_split_vardct_source_ppm(&input);
    ...
    assert_eq!(
        xyb_summary,
        vec![
            (61440, 6103996420772725586),
            (61440, 17974925784563451160),
            (61440, 10284642808155675766),
        ]
    );
    assert_eq!(
        xyb_anchors,
        [
            3099662244, 1003517621, 1002474870, 969377705, 1053818599, 1053956526
        ]
    );
    assert_eq!(
        rgb_summary,
        vec![
            (61440, 6005004399905771653),
            (61440, 17793056164799520906),
            (61440, 2612738635243201652),
        ]
    );
    assert_eq!(
        rgb_anchors,
        [
            967901028, 974062018, 971944974, 1043931543, 1043239635, 1043827192
        ]
    );
    assert_eq!(metrics.max_abs_error, 255);
    assert_eq!(metrics.sum_abs_error, 7705630);
    assert_eq!(metrics.checksum, 17160417689224148714);
    assert_eq!(metrics.anchors, vec![1, 2, 2, 255, 255, 255, 118, 115, 117]);
    assert_eq!(
        metrics.reference_anchors,
        vec![0, 1, 1, 125, 128, 124, 253, 255, 255]
    );
...
fn write_flat_vardct_source_ppm(path: &Path) {
    let width = 64u32;
    let height = 64u32;
    let mut bytes = format!("P6\n{width} {height}\n255\n").into_bytes();
...
fn parses_checked_in_fixture_vardct_metadata() {
    ...
    assert_eq!(vardct_plan.dc_group_payloads.len(), 1);
    assert_eq!(vardct_plan.ac_group_payloads.len(), 1);
